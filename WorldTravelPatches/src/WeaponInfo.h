...
// Taken from FiveM; https://github.com/citizenfx/fivem/blob/master/code/components/gta-streaming-five/src/PatchWeaponLimits.cpp
...
static void RelocateRelative(std::initializer_list<PatternPair> list)
{
	...
	for (auto& entry : list)
	{
		...
		intptr_t relativeOffset = (intptr_t)weaponComponentInfoCollection - (intptr_t)location - 4;

		assert(relativeOffset >= INT32_MIN && relativeOffset <= INT32_MAX);

		hook::put<int32_t>(location, static_cast<int32_t>(relativeOffset));
		++index;
	}
}
